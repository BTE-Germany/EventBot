        ...
        if (args.channel.id === process.env.SUBMISSION_CHANNEL) {
            ...
            if (!user) {
                const botmessage = await args.channel.send({
                    content: "You are not registered! Please register with `/register`.", messageReference: {
                        messageID: args.id
                    }
                });
                setTimeout(async () => {
                ...
                if (args.attachments.map(a => a).length > 0 && args.content.length > 0) {
                    prisma.build
                        .create({
                            data: {
                        ...
                        .then(async (obj) => {
                            ...
                            for (const image of args.attachments.map(a => a).slice(0, 3)) {
                                ...
                                const blockBlobClient = containerClient.getBlockBlobClient(`${user.id}/${uuid}${image.url.substring(image.url.lastIndexOf("."))}`);
                                await blockBlobClient.uploadData(buffer, buffer.byteLength);
                                embeds.push({
                                    url: "https://bte-germany.de", image: {
                                        url: `${process.env.CDN_URL}/${process.env.CONTAINER_NAME}/${user.id}/${uuid}${image.url.substring(image.url.lastIndexOf("."))}`,
                                    }
                                });
                                images.push(`${process.env.CDN_URL}/${process.env.CONTAINER_NAME}/${user.id}/${uuid}${image.url.substring(image.url.lastIndexOf("."))}`);
                            }
                            ...
                            args.client.channels.cache.get(process.env.JUDGE_CHANNEL).send({
                            ...
                            await args.delete();
                            console.log(new Date().toLocaleString(), `Created new build by ${args.author.username}#${args.author.discriminator} with id ${obj.id}`);
                        });
