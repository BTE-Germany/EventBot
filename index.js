...
for (const file of eventFiles) {
    ...
    if (event.name !== "ready" && event.name !== "interactionCreate") {
        if (event.once) {
            console.log(new Date().toLocaleString(), `Registering event: ${event.name}`);
...
client.once('ready', async () => {
    ...
    for (const file of commandFiles) {
        ...
        commands.push(data);
        client.application.commands.create(data.command).then(() => console.log(new Date().toLocaleString(), `Created command /${data.command.name}`)).catch(console.error);
    }

    const scheduleFiles = fs.readdirSync('./schedule').filter(file => file.endsWith('.js'));
    for (const file of scheduleFiles) {
        let data = require(`./schedule/${file}`);
        console.log(new Date().toLocaleString(), `registering schedule: ${file} (${data.time}ms)`);
        setInterval(() => {
