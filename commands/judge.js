...
module.exports = {
    command: {
        name: "judge", description: "Judge a build!", options: [{
            name: "id", description: "The ID of the build you want to delete.", type: 4, required: true,
        }, {
            name: "details",
        ...
        if (await prisma.judge.findUnique({
            ...
            if (!build) {
                ...
                if (build.judges?.length === 0) {
                    ...
                    const base_points = (typeof interaction.options.getBoolean("grundpunkte") === "boolean") ? interaction.options.getBoolean("grundpunkte") : true;
                    ...
                    const User = await interaction.guild.members.cache.get(build.builder_id.toString()).user;
                    let embeds = [{
                    ...
                    build.images.forEach((image) => {
                        embeds.push({
                            url: "https://bte-germany.de", image: {
                                url: image,
                            }
                        });
                    });
                ...
                if (build.judges?.length === 1) {
                    ...
                    await prisma.user.update({
                        where: {
                            ...
                            points: user?.points + ((build.A + interaction.options.getInteger("details")) / 2) + ((build.B + interaction.options.getInteger("aufwand")) / 2) + base_points,
                        },
                    });
                    interaction.reply("Build bewertet. Punkte wurden dem User gutgeschrieben. Du warst der 2. Judge. Somit wurde deine Entscheidung für base_points ignoriert.")
                    const User = await interaction.guild.members.cache.get(build.builder_id.toString()).user;
                    let embeds = [{
                        title: `#${build.id.toString()}`,
                        description: `Koordinaten: ${build.location}`,
                    ...
                    await client.channels.cache.get(process.env.SUBMISSION_CHANNEL).messages.fetch(build.message.toString()).then(message => {
                        message.edit({
                            content: " ", embeds: embeds
                        });
                    });
